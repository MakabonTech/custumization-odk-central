...
RUN git describe --tags --dirty > /tmp/sentry-versions/central || echo "no tag" > /tmp/sentry-versions/central
WORKDIR /server
RUN git describe --tags --dirty > /tmp/sentry-versions/server || echo "no tag" > /tmp/sentry-versions/server
WORKDIR /client
RUN git describe --tags --dirty > /tmp/sentry-versions/client || echo "no tag" > /tmp/sentry-versions/client
