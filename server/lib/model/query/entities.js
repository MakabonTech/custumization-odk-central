...
const restore = (entity) => ({ run }) =>
  /* We're not simply running markUndeleted() because markUndeleted() doesn't
  bump updatedAt. When an entity is restored, that needs to have the effect of
  changing the OpenRosa hash of the dataset. For that to happen for ownerOnly
  datasets, the updatedAt timestamp needs to change. Note that in general, we
  bump entities.updatedAt not only when there's a new entity version, but also
  when metadata about the entity changes, e.g., the entity's conflict status.
  Restoring an entity changes the entity's deletedAt timestamp, which can be
  thought of as part of its metadata. */
  run(sql`
  UPDATE entities
  SET "deletedAt" = null, "updatedAt" = clock_timestamp()
  WHERE id = ${entity.id}`);
  ...
  return oneFirst(sql`
    WITH redacted_audits AS (
      ...
      INSERT INTO audits ("action", "acteeId", "loggedAt", "processed", "details")
        SELECT 'entity.purge', datasets."acteeId", clock_timestamp(), clock_timestamp(), jsonb_build_object('entitiesDeleted', COUNT(*))
        FROM entities
        JOIN datasets ON entities."datasetId" = datasets.id
      ...
      INSERT INTO purged_entities ("entityUuid", "acteeId", "auditId")
        SELECT entities.uuid, datasets."acteeId", purge_audit.id
        FROM entities, datasets, purge_audit
        WHERE entities."datasetId" = datasets.id
        AND datasets."acteeId" = purge_audit."acteeId"
        AND ${_trashedFilter(force, projectId, datasetName, entityUuid)}
...
const getEntitiesState = (dataset, options = QueryOptions.none) => ({ all }) => all(sql`
  ...
  UNION
  SELECT "entityUuid" AS uuid, TRUE AS deleted FROM purged_entities
  WHERE "acteeId" = ${dataset.acteeId}
  AND ${idFilter(options, 'entityUuid')}
